...
# Load the dataset
df = pd.read_csv('overall_sales.csv')
...
X_train, y_train = train_df[['MonthsSinceStart']], train_df['Overall Sales']
X_test, y_test = test_df[['MonthsSinceStart']], test_df['Overall Sales']
...
# Calculate errors
errors_df = test_df[['Overall Sales']].copy()
...
ax.plot(errors_df.index, errors_df['Overall Sales'], label='Average Data')
...
plt.plot(df.index, df['Overall Sales'], label='Actual Data')
...
forecast_df = pd.DataFrame({'Year Month': forecast_dates, 'Overall Sales': forecast_predictions})
...
combined_df.to_csv('overall_sales_forecast.csv', index=False)