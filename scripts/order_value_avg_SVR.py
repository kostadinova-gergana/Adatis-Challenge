...
# Load the dataset
df = pd.read_csv('order_value_avg.csv')
...
X_train, y_train = train_df[['MonthsSinceStart']], train_df['Average Order Value']
X_test, y_test = test_df[['MonthsSinceStart']], test_df['Average Order Value']
...
# Calculate errors
errors_df = test_df[['Average Order Value']].copy()
...
ax.plot(errors_df.index, errors_df['Average Order Value'], label='Average Data')
ax.plot(errors_df.index, errors_df['pred_sales'], label='Forecast')
ax.legend(loc='best')
ax.set_xlabel('Date')
ax.set_ylabel('Average Order Amount')
ax.set_title('SVR forecasts with actual average order amounts and errors')
plt.show()
...
plt.plot(df.index, df['Average Order Value'], label='Actual Data')
...
plt.title('Average Order Amount Forecasting with Support Vector Regression')
plt.xlabel('Date')
...
forecast_df = pd.DataFrame({'Year Month': forecast_dates, 'Average Order Value': forecast_predictions})
...
combined_df.to_csv('order_value_avg_forecast.csv', index=False)