...
# Load the dataset
df = pd.read_csv('order_amount_avg.csv')
...
X_train, y_train = train_df[['MonthsSinceStart']], train_df['AVG_OrderAmount']
X_test, y_test = test_df[['MonthsSinceStart']], test_df['AVG_OrderAmount']
...
# Calculate errors
errors_df = test_df[['AVG_OrderAmount']].copy()
...
ax.plot(errors_df.index, errors_df['AVG_OrderAmount'], label='Average Data')
...
plt.plot(df.index, df['AVG_OrderAmount'], label='Actual Data')
...
forecast_df = pd.DataFrame({'Year Month': forecast_dates, 'AVG_OrderAmount': forecast_predictions})
...
combined_df.to_csv('order_amount_avg_forecast.csv', index=False)