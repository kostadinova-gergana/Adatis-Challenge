...
X_train, y_train = train_df[['MonthsSinceStart']], train_df['Sales Growth MoM']
X_test, y_test = test_df[['MonthsSinceStart']], test_df['Sales Growth MoM']
...
# Calculate errors
errors_df = test_df[['Sales Growth MoM']].copy()
...
ax.plot(errors_df.index, errors_df['Sales Growth MoM'], label='Average Data')
...
plt.plot(df.index, df['Sales Growth MoM'], label='Actual Data')
...
forecast_df = pd.DataFrame({'Year Month': forecast_dates, 'Sales Growth MoM': forecast_predictions})
...
combined_df.to_csv('sales_growth_forecast.csv', index=False)