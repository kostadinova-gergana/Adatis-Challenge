...
# Load the dataset
df = pd.read_csv('units_per_transaction.csv')
...
# Number of months in the training set
num_months_training = (cutoff_date - df.index.min()).days // 30

# Number of months in the testing set
num_months_testing = (df.index.max() - cutoff_date).days // 30

print("Number of months in the training set:", num_months_training)
print("Number of months in the testing set:", num_months_testing)

# Prepare data for training
X_train, y_train = train_df[['MonthsSinceStart']], train_df['Units Per Transaction']
X_test, y_test = test_df[['MonthsSinceStart']], test_df['Units Per Transaction']
...
# Calculate errors
errors_df = test_df[['Units Per Transaction']].copy()
...
ax.plot(errors_df.index, errors_df['Units Per Transaction'], label='Average Data')
...
plt.plot(df.index, df['Units Per Transaction'], label='Actual Data')
...
forecast_df = pd.DataFrame({'Year Month': forecast_dates, 'Units Per Transaction': forecast_predictions})
...
combined_df.to_csv('units_per_transaction_forecast.csv', index=False)