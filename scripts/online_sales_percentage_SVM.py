...
# Load the dataset
df = pd.read_csv('online_sales_percentage.csv')
...
X_train, y_train = train_df[['MonthsSinceStart']], train_df['OnlineSalesPercentage']
X_test, y_test = test_df[['MonthsSinceStart']], test_df['OnlineSalesPercentage']
...
# Calculate errors
errors_df = test_df[['OnlineSalesPercentage']].copy()
...
ax.plot(errors_df.index, errors_df['OnlineSalesPercentage'], label='Actual Data')
ax.plot(errors_df.index, errors_df['pred_sales'], label='Forecast')
ax.legend(loc='best')
ax.set_xlabel('Date')
ax.set_ylabel('Online Sales Percentage')
ax.set_title('SVR forecasts with actual sales and errors')
plt.show()
...
plt.plot(df.index, df['OnlineSalesPercentage'], label='Actual Data')
...
forecast_df = pd.DataFrame({'Year Month': forecast_dates, 'OnlineSalesPercentage': forecast_predictions})
...
combined_df.to_csv('online_sales_percentage_forecast.csv', index=False)